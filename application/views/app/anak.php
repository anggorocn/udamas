...
$arrtabledata= array(
    ...
    , array("label"=>"Status Keluarga", "field"=> "STATUS_KELUARGA_NAMA", "display"=>"", "width"=>"")
    , array("label"=>"Status Aktif", "field"=> "STATUS_NAMA", "display"=>"", "width"=>"")
...
$arrparam= ["token"=>$sessPersonalToken, "vurl"=>"anak_json"];
$set= new DataCombo();
              ...
              <?
                  ...
                  else if($field == "AKSI")
                  {
                   if(strtoupper($vaksesmenu) == strtoupper("D")){}
                    ...
                    {
                    ?>
                      <button class="btn btn-xs btn-danger" type="button" onclick="hapusdata('<?=$reqRowId?>', 'anak', '', '<?=$linkfilename?>')">Batal</button>
                    <?
                    }
                    if(!empty($reqHapusId))
                    {
                    ?>
                      <button class="btn btn-xs btn-danger" type="button" onclick="hapusdata('<?=$reqHapusId?>', 'anak', '2', '<?=$linkfilename?>')">Batal</button>
                    <?
                    }
                  }
                  }
