...
$arrtabledata= array(
    ...
    , array("label"=>"Kredit Utama", "field"=> "KREDIT_UTAMA", "display"=>"", "width"=>"")
    , array("label"=>"Kredit Penunjang", "field"=> "KREDIT_PENUNJANG", "display"=>"", "width"=>"")
    , array("label"=>"Total", "field"=> "TOTAL_KREDIT", "display"=>"", "width"=>"")
...
$arrparam= ["token"=>$sessPersonalToken, "vurl"=>"pak_json"];
$set= new DataCombo();
            ...
            <tr>
              ...
              <?
                  ...
                  else if($field == "AKSI")
                  {
                    ...
                    {
                    ?>
                      <button class="btn btn-xs btn-danger" type="button" onclick="hapusdata('<?=$reqRowId?>', 'pak', '', '<?=$linkfilename?>')">Batal</button>
                    <?
                    }
                    if(!empty($reqHapusId))
                    {
                    ?>
                      <button class="btn btn-xs btn-danger" type="button" onclick="hapusdata('<?=$reqHapusId?>', 'pak', '2', '<?=$linkfilename?>')">Batal</button>
                    <?
                    }
                  }
                  }
